...
export async function submit({ apiKey, prompt, imageBuffer, imageMime, duration, aspectRatio, resolution, model }) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`BytePlus ${res.status}: ${text.slice(0, 300)}`);
  }
  const data = await res.json();
  const providerRequestId = data.id;
  if (!providerRequestId) {
    throw new Error(`BytePlus: no task id in response: ${JSON.stringify(data).slice(0, 200)}`);
  }
...
export async function pollResult({ apiKey, providerRequestId }) {
  ...
  if (TERMINAL_OK.has(s)) {
    ...
    if (!videoUrl) {
      return { status: 'failed', error: `BytePlus succeeded but no video URL found: ${JSON.stringify(data).slice(0, 300)}` };
    }
    return { status: 'done', videoUrl };
  }
  if (TERMINAL_FAIL.has(s)) {
    return { status: 'failed', error: data.error?.message || data.message || `BytePlus status: ${s}` };
  }
