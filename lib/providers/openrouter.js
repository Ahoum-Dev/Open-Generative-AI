...
export async function submit({ apiKey, prompt, imageBuffer, imageMime, duration, aspectRatio, resolution, model }) {
  ...
  if (!providerRequestId) {
    throw new Error(`OpenRouter: no id in response: ${JSON.stringify(data).slice(0, 200)}`);
  }
...
export async function pollResult({ apiKey, providerRequestId }) {
  ...
  if (TERMINAL_OK.has(s)) {
    ...
    if (!sourceUrl) {
      return { status: 'failed', error: `OpenRouter completed but no unsigned_urls: ${JSON.stringify(data).slice(0, 300)}` };
    }
  ...
  if (TERMINAL_FAIL.has(s)) {
    return { status: 'failed', error: data.error?.message || data.message || `OpenRouter status: ${s}` };
  }
