...
// Hard ceiling on total submit-to-done wall-clock for a single job. A 15s
// Seedance i2v typically completes in 1-3 minutes; 30 is comfortably above
// that and well below "stuck forever". The worker terminal-fails the job
// past this and surfaces it to the user as "check the dashboard before retry".
export const maxPollMinutes = 30;
...
export async function submit({ apiKey, prompt, imageBuffer, imageMime, imageFileName, duration, quality, aspectRatio, model }) {
  ...
  if (!res.ok) {
    ...
    const msg = `MuAPI ${res.status}: ${text.slice(0, 200)}`;
  ...
  if (!providerRequestId) {
    throw new TerminalProviderError(
      `MuAPI: no request_id in response: ${JSON.stringify(data).slice(0, 200)}`,
      { billed: null },
    );
  }
...
export async function pollResult({ apiKey, providerRequestId }) {
  ...
  if (!res.ok) {
    ...
    const text = await res.text().catch(() => '');
    throw new TerminalProviderError(
      `MuAPI poll ${res.status}: ${text.slice(0, 200)}`,
      { billed: null },
    );
  }
  const data = await res.json();
