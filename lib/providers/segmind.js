...
// Segmind is sync (response IS the video), so polling never runs in the
// happy path. Set anyway as a safety net in case a job somehow ends up in
// 'polling' state through future refactor.
export const maxPollMinutes = 10;
...
export async function submit({ apiKey, prompt, imageBuffer, imageMime, jobId, duration, aspectRatio, resolution, model }) {
  ...
  if (!uploadRes.ok) {
    ...
    const msg = `Segmind upload-asset ${uploadRes.status}: ${text.slice(0, 200)}`;
  ...
  if (!firstFrameUrl) {
    throw new TerminalProviderError(
      `Segmind upload-asset returned no url: ${JSON.stringify(uploadJson).slice(0, 200)}`,
      { billed: false },
    );
  }
  ...
  if (!genRes.ok) {
    ...
    const msg = `Segmind ${model || defaultModel} ${genRes.status}: ${text.slice(0, 200)}`;
    if (genRes.status >= 400 && genRes.status < 500) {
      throw new TerminalProviderError(msg, { billed: false });
    }
    throw new Error(msg);
  }
  ...
  if (!contentType.startsWith('video/')) {
    // Segmind returns 2xx + JSON on policy violations. The generation was
    // started and likely billed before the policy check rejected it, so
    // mark terminal — retrying will hit the same policy and re-charge.
    const text = await genRes.text().catch(() => '');
    throw new TerminalProviderError(
      `Segmind returned non-video content-type "${contentType}": ${text.slice(0, 300)}`,
      { billed: true },
    );
  }
